...
- Helm Chart 내용
  ...
  - Deployment를 이용해 배포 해야합니다.
    1. `spec.containers[0].image`는 `"{{ .Values.image.repository }}:{{ .Values.image.tag | default .Chart.AppVersion }}"` 형태로 작성되어야 합니다
  - Service를 이용해 서비스를 노출 해야합니다.
    1. Service Type은 NodePort로 작성해야합니다.
    2. NodePort는 30080을 사용해야합니다.
    3. kubernetes endpoint:30080/healthcheck를 통해 정상적으로 응답을 받아야합니다.
